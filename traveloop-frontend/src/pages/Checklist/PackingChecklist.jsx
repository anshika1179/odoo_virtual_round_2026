...
import { getChecklist, createChecklistItem, updateChecklistItem, deleteChecklistItem, resetChecklist } from '../../services/api';
import { CheckSquare, Square, Plus, Trash2, RotateCcw, Search, Printer, Loader2 } from 'lucide-react';
      ...
      <div className="animate-fadeInUp">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400">{packedCount}/{items.length} items packed</p>
          </div>
        ...
        <div className="glass rounded-2xl p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-slate-400">Packing Progress</span>
            <span className="text-sm text-indigo-300 font-medium">{items.length > 0 ? Math.round(packedCount / items.length * 100) : 0}%</span>
          </div>
          <div className="h-3 rounded-full bg-slate-700 overflow-hidden">
            <div className="h-full rounded-full bg-gradient-to-r from-indigo-500 to-green-500 transition-all duration-500"
        ...
        <div className="space-y-6">
          {CATEGORIES.filter(cat => grouped[cat]?.length > 0).map(cat => (
            <div key={cat} className="glass rounded-2xl overflow-hidden">
              <div className="bg-gradient-to-r from-indigo-500/10 to-purple-500/10 px-6 py-3 flex items-center gap-2">
                <span className="text-lg">{CAT_ICONS[cat]}</span>
                <h3 className="text-white font-semibold">{cat}</h3>
                <span className="text-xs text-slate-400 ml-auto">{grouped[cat].filter(i => i.is_packed).length}/{grouped[cat].length}</span>
              </div>
                ...
                {grouped[cat].map(item => (
                  <div key={item.id} className="flex items-center gap-3 px-6 py-3 hover:bg-white/[0.02] transition-colors group">
                    <button onClick={() => handleToggle(item)} className="shrink-0">
                      {item.is_packed ? <CheckSquare size={20} className="text-green-400" /> : <Square size={20} className="text-slate-500" />}
                    </button>
                    <span className={`flex-1 text-sm ${item.is_packed ? 'text-slate-500 line-through' : 'text-slate-300'}`}>{item.item_name}</span>
                    <button onClick={() => handleDelete(item.id)} className="p-1 rounded text-slate-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
            </div>
